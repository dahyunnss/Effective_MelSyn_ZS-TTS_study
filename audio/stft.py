...
class STFT(torch.nn.Module):
    ...
    def transform(self, input_data):        
        ...
        # 입력 데이터 크기에 따른 안전한 패딩 크기 계산
        max_pad_len = min(int(self.filter_length / 2), num_samples - 1) #min(512, num_samples -1)

        # 입력 데이터에 적용
        input_data = input_data.view(num_batches, 1, num_samples)
        
        # reflect 패딩 적용, 조정된 패딩 크기 사용
        input_data = F.pad(
                input_data.unsqueeze(1),(max_pad_len, max_pad_len, 0, 0), mode='reflect')
        
        # input_data = F.pad(
        #     input_data.unsqueeze(1),
        #     (int(self.filter_length / 2), int(self.filter_length / 2), 0, 0), #error
        #     mode='reflect')
        input_data = input_data.squeeze(1)
        
        forward_transform = F.conv1d( #gpu
            input_data.cuda(),
...
class TacotronSTFT(torch.nn.Module):
    ...
    def mel_spectrogram(self, y):
        ...
        device = y.device
        mel_basis = self.mel_basis.to(device)
